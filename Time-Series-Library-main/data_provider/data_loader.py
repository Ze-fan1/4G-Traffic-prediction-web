...
class Dataset_Custom(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
        ...
        # ─────── 【核心修改1】根据不同的阶段，强制指定独立的文件名 ───────
        # ─────── 【把 val 也指向 test 文件】 ───────
        if self.set_type == 0:
            self.data_path = 'df_4g_train_100.parquet'
        elif self.set_type == 1:
            self.data_path = 'df_4g_train_100.parquet'
        else:
            self.data_path = 'df_4g_test_100.parquet'  
        # ─────────────────────────────────────────────────────────────
    ...
    def __read_data__(self):
        ...
        if self.data_path.endswith('.parquet'):
            df_raw = pd.read_parquet(local_fp)
        else:
            df_raw = pd.read_csv(local_fp)

        # ─────── 核心数据清洗：动态扔掉不能预测的非数值/静态列 ───────
        ...
        if self.scale:
            if self.set_type == 0:
                ...
                date_bounds = sorted(dates.unique())
                split_idx = int(len(date_bounds) * 0.8)
                train_cutoff = date_bounds[split_idx]
                ...
                # 测试/验证集必须隐式加载训练集来 fit 标准差，再 transform 自身
                train_fp = os.path.join(self.root_path, 'df_4g_train_100.parquet')
                ...
                if self.set_type == 1:
                    ...
                    date_bounds = sorted(dates.unique())
                    split_idx = int(len(date_bounds) * 0.8)
                    val_cutoff = date_bounds[split_idx]
                    val_mask = dates >= val_cutoff
                    df_raw = df_raw[val_mask].reset_index(drop=True)
                    df_data = df_raw[df_raw.columns[1:]]
        ...
        # 步长逻辑保留
        self.step = 3 if self.set_type == 2 else 1
